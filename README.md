...
## Release Note
### v1.1
- Previously, the %Lcis, %Scis and %Trans (%inter) were calculated based on the total number of unique valid pairs. We changed the calculation in this version in order to make it consistent with Juicer pipeline's calculation (%Lcis = Lcis / uniquely mapped reads). We now use unique total pairs (no matter valid or invalid) as the denominator instead of the unique valid pairs. Please do not compare those percentage values directly generated by this version and the previous version.
- Other minor fixes
